...
void loli_ci_init(loli_code_iter *iter, uint16_t *buffer, uint16_t start,
        uint16_t stop)
{
    iter->buffer = buffer;
    iter->stop = stop;
...
void loli_ci_from_native(loli_code_iter *iter, loli_function_val *fv)
{
    iter->buffer = fv->code;
...
int loli_ci_next(loli_code_iter *iter)
{
    ...
    memset(&iter->round_total, 0,
            sizeof(loli_code_iter) - offsetof(loli_code_iter, round_total));
